...
function pageStep(stepId){ const p=getSession(); if(!p){ location.href='intro.html'; return;} const meta=ensureMeta(); const ended=isEnded(meta); const step=GAME.steps.find(s=>s.id===stepId); const players=loadPlayers(); const st=players[p]||{found:[],score:0}; const found=new Set(st.found||[]); const total=GAME.steps.length; const val=(st.found||[]).length; const pct=Math.round(val/total*100); document.getElementById('player').innerHTML = esc(p); document.getElementById('progressBar').style.width=pct+'%'; document.getElementById('progressTxt').textContent=`${val}/${total} codes`; document.getElementById('deadline').textContent=new Date(meta.endAt).toLocaleString('fr-FR'); if(ended){ document.getElementById('codeInput').disabled=true; document.getElementById('btnValider').disabled=true; } document.getElementById('btnPrev').onclick=()=>{ const i=GAME.steps.findIndex(s=>s.id===stepId); location.href=(GAME.steps[i-1]?.id||'egy1')+'.html'; }; document.getElementById('btnNext').onclick=()=>{ const i=GAME.steps.findIndex(s=>s.id===stepId); location.href=(GAME.steps[i+1]?.id||'finale')+'.html'; }; document.getElementById('btnHome').onclick=()=> location.href='index.html'; document.getElementById('msg').textContent=step.msg; const fb=document.getElementById('feedback'); document.getElementById('btnValider').onclick=()=>{ if(ended){ fb.textContent='Jeu terminé — en pause.'; fb.className='bad'; return;} const valIn=sanitize(document.getElementById('codeInput').value); const target=sanitize(step.word); const nowISO=toISO(now()); if(!valIn){ fb.textContent='Entre un mot-code.'; fb.className='bad'; return;} if(valIn===target){ if(!found.has(step.id)){ st.found=[...new Set([...(st.found||[]), step.id])]; st.score=(st.score||0)+(step.points||1); st.lastTs=nowISO; st.journal=[...(st.journal||[]), {stepId:step.id,word:step.word,ts:nowISO}]; players[p]=st; savePlayers(players); } fb.textContent='✅ Correct (+1)'; fb.className='ok'; const newPct=Math.round((st.found.length)/total*100); document.getElementById('progressBar').style.width=newPct+'%'; document.getElementById('progressTxt').textContent=`${st.found.length}/${total} codes`; }else{ fb.textContent='❌ Mauvais code'; fb.className='bad'; } } }
...
// Auto-init by page type
document.addEventListener('DOMContentLoaded', ()=>{
  const page = document.body.dataset.page;
  if(page==='index') pageIndex();
  else if(page==='intro') pageIntro();
  else if(page==='step') pageStep(document.body.dataset.step);
  else if(page==='finale') pageFinale();
});
